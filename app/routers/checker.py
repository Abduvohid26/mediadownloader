...
# test_url = "https://vt.tiktok.com/ZSr9A2KyN/"
# # test_url = "https://www.instagram.com/stories/the.nodirchik"

# async def fetch(session, idx):
#     try:
#         async with session.get(API_URL, params={"tk_url": test_url}, timeout=30) as response:
...
#         tasks = [fetch(session, i + 1) for i in range(15)]
...
async def main():
    async with aiohttp.ClientSession() as session:
        for minute in range(1):  # Run for 1 minute
            tasks = [fetch(session, i + 1) for i in range(2)]  # Create 2 tasks per second
